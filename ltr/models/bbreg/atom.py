...
@model_constructor
def atom_resnet50_fpn(iou_input_dim=(256, 256), iou_inter_dim=(256, 256), backbone_pretrained=True, share_rt=False):
    # backbone
    backbone_net = backbones.resnet50(pretrained=backbone_pretrained)

    # Bounding box regressor
    iou_predictor = bbmodels.AtomFPNIoUNet(
    input_dim=(512, 1024),
    pred_input_dim=iou_input_dim,
    pred_inter_dim=iou_inter_dim,
    share_rt=share_rt,
...
@model_constructor
def atom_resnet50_mul_fpn(iou_input_dim=(256, 256), iou_inter_dim=(256, 256), backbone_pretrained=True, share_rt=False):
    # backbone
    backbone_net = backbones.resnet50(pretrained=backbone_pretrained)
