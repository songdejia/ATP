function [bboxes]= optimize_bboxes(seg,gt)

    seg =reshape(seg,sqrt(size(seg,2)),sqrt(size(seg,2)))';
...
%     scores_gt = nan(size_images, 2);
    ...
    x_img = repmat([(1):(size(seg,2))], [length([1:size(seg,1)]) 1]);
    y_img = repmat([(1):(size(seg,1))]', [1 length([1:size(seg,2)])]);
    consts.xq = x_img(:, :);
    consts.yq = y_img(:, :);
    ...
    if length(gt) == 4
        bb_gt = [gt(1), gt(2), gt(1)+gt(3), gt(2), gt(1)+gt(3), gt(2)+gt(4), gt(1), gt(2)+gt(4)];
    else
        bb_gt = gt;
    end
...
%     end
    ...
    costs_gt = cost_fnc([cx, cy, width, height, angle], seg, consts);
    ...
    if (exitflag_c <= -2 && costs_gt <= fval)
        bboxes = order_bbox_corners(initial_bbox(1,:));
        %bboxes=mat2str(bboxes);
    else
        bboxes = params2corners(x);
        %bboxes=mat2str(bboxes);
    end
end
