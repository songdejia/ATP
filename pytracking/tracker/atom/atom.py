...
class ATOM(BaseTracker):
    ...
    def extract_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        # original only return sample_feature
        # self.params.features.extract(im, pos, scales, sz)
        sample_img, sample_feature = self.params.features.extract_v2(im, pos, scales, sz)
    ...
    def update_memory(self, sample_x: TensorList, sample_y: TensorList, learning_rate = None):
        ...
        # modify: for different size training feature sample, resize to the formmer shape
        for train_samp, x, ind in zip(self.training_samples, sample_x, replace_ind):
            if x.shape[-1] == train_samp[0].shape[-1]:
                train_samp[ind:ind+1,...] = x
            else:
                template_shape = self.training_samples[0].shape[2:]
                tx = x.squeeze().cpu().numpy().copy()
                txs = [cv2.resize(tx[i], template_shape)[np.newaxis, :, :] for i in range(tx.shape[0])]
                x = torch.from_numpy(np.concatenate(txs, axis = 0)[np.newaxis, :, :, :]).cuda()
                train_samp[ind:ind+1,...] = x

        for y_memory, y, ind in zip(self.y, sample_y, replace_ind):
            if y.shape[-1] == y_memory[0].shape[-1]:
                y_memory[ind:ind+1,...] = y
            else:
                template_shape = self.y[0].shape[2:]
                ty = y.squeeze().cpu().numpy().copy()
                y = cv2.resize(ty, template_shape)[np.newaxis, np.newaxis, :, :]
